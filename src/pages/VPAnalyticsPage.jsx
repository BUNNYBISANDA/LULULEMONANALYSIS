...
export default function VPAnalytics() {
  ...
  const executiveInsight = useMemo(() => {
    ...
    const labels = topCategories.map((row) => row.label)
    const categoryNames =
      labels.length > 1
          ...
          <div className="flex min-w-0 items-start gap-3 sm:gap-4">
            ...
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-[#4a4a4a]">
                What guests are telling us, the evidence behind it, and what the factory should
                do next.
              </p>
            </div>
          </div>
      ...
      {hasReviews ? (
        <>
          <Panel className="p-4 sm:p-6 lg:p-7">
            ...
            <div className="mt-5 flex items-start gap-3 rounded-[18px] border border-[#f1c7cb] bg-[#fff9fa] p-4 sm:gap-4 sm:p-5">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white text-[#E20010]">
                <Lightbulb size={18} />
              </span>
              <p className="font-display text-base font-semibold leading-7 text-black sm:text-lg">
                {executiveInsight}
              </p>
            </div>
          </Panel>
