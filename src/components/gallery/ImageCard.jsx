...
export default function ImageCard({ item, extraCount = 0, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="group overflow-hidden rounded-[20px] border border-[#e5e5e5] bg-white text-left transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_10px_28px_rgba(0,0,0,0.08)]"
    >
      <div className="relative aspect-square overflow-hidden bg-[#f5f5f5]">
        ...
        <div className="absolute inset-x-0 bottom-0 flex items-end justify-between bg-gradient-to-t from-black/65 via-black/10 to-transparent p-4 text-white opacity-0 transition group-hover:opacity-100">
          <div>
            <p className="font-display text-sm font-semibold">{item.reviewTitle}</p>
